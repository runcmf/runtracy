    ...
    public function getPanel()
    {
        ...
        <script>
            ...
            function changeTracyPanels(type) {
                document.cookie = "tracyPanels"+type+"="+JSON.stringify( getSelectedTracyPanelCheckboxes() )+"; expires=0; path=/";
                location.reload();
            }
        ...
        foreach ($this->defcfg as $name => $val) {
            if ($name === 'XDebugHelperIDEKey' || $name === 'showPanelSelector') {
                continue;
            }
            $out .= '<tr><td style="padding-left: 20px">
            <input type="checkbox" name="selectedPanels[]" ' . (($this->cfg[$name] == 1) ? 'checked="checked"' : '') . ' value="' . $name . '" />
